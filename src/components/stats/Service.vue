<template lang="pug">
	.container
		.justification(v-for="service in services" @click="dynamicDialog(service)")
			img.pr-3(:src="service.url" width=158 height=138)
			span.justification-span {{ service.title }}
			.text
				span {{service.value}} resp. | {{ service.stats.satisfaction || 0 }}% satis.
			...
			v-card
				...
				v-card-text
					v-flex(xs9 offset-xs2)
						Face#serviceFaces.pb-5(:data="currentService.stats.indicatorsGlobal.faces")
					v-flex(xs3 offset-xs10)
						v-tooltip(top)
							v-btn(
								...
								color="primary"
								slot="activator"
								@click="printGeneralReport(currentService.stats.assessments)"
								:loading="loading2"
								:disabled="loading2"
							)
						...
						v-tooltip(top)
							v-btn(
								...
								color="primary"
								slot="activator"
								@click="downloadXLSX(currentService.stats.assessments)"
								:loading="loading3"
								:disabled="loading3"
							)
								v-icon(dark) storage
					...
					v-tabs(fixed centered)
						...
						v-tabs-items
							v-tabs-content(id="hourService")
								v-flex.py-5(xs12 v-if="chartHourGlobal")
									Chart#chartHourServiceOrigin(type="columnStacked" title="Distribución General Horaria" :data="chartHourGlobal")
	...
	export default {
		...
		data () {
			return {
				...
				services: {
					0: { index: 0, ftitle: 'Pago de servicios', url: "./../../../static/services/pago-servicios.png", title: 'Pago Servicios', value: 0, percentage: 0, stats: {} },
					1: { index: 1, ftitle: 'Envio internacional', url: "./../../../static/services/envio-internacional.png", title: 'Envío Internacional', value: 0, percentage: 0,  stats: {} },
					2: { index: 2, ftitle: 'Envio nacional', url: "./../../../static/services/pago-servicios.png", title: 'Envío Nacional', value: 0, percentage: 0,  stats: {} },
					3: { index: 3, ftitle: 'Casa de Cambio', url: "./../../../static/services/casa-cambio.png", title: 'Casa de Cambio', value: 0, percentage: 0, stats: {} },
				},
			}
		},
		watch: {
			data() {
				if (this.$props.data) {
					...
					for (let service in this.services) {
						index = this.services[service].index
						this.services[service].value = this.$props.data[index][0]
						this.services[service].percentage = this.$props.data[index][1]
						this.services[service].stats = this.$props.data[index][2]
					}
				}
			},
      loader2 () {
		...
		methods: {
			async printGeneralReport(data) {
				...
				const FILENAME  = 'reporte_servicio.pdf'
				let pdf = new jsPDF()
        ...
        pdf.setFontSize(20)
        if (this.currentService.title !== 'Envío Internacional') pdf.text(`Reporte de Valoraciones (${this.currentService.title})`, 40, 150)
        else pdf.text(`Reporte de Valoraciones (${this.currentService.title})`, 35, 150)
				pdf.setFontSize(12)
				...
				const servicesFacesData2 = document.getElementById('serviceFaces2')
        ...
        const serviceFaces = await getCanvas('serviceFaces2')
				...
				const servicesIndSatisfaction = document.getElementById('satisfactionIndService')

				servicesIndSatisfaction.style.display = "block"
				servicesIndSatisfaction.childNodes[0].children[0].children[0].children[0].children[0].children[0].children[0].innerHTML = `Satisfacción del Cliente ${this.currentService.stats.satisfaction}%`

				const satisfactionInd = await getCanvas('satisfactionIndService')
				pdf.addImage(satisfactionInd.toDataURL('image/png'), 'PNG', 10, 70, 190, 30)

				const servicesIndicators = document.getElementById('indicatorsService')

				servicesIndicators.style.display = "flex"

				servicesIndicators.childNodes[0].children[0].children[0].children[0]
				.children[0].children[0].children[0].children[0].innerHTML = `Quejas ${this.currentService.stats.indicatorsGlobal.complain[1]}% - Total ${this.currentService.stats.indicatorsGlobal.complain[0]}`

				servicesIndicators.childNodes[1].children[0].children[0].children[0]
				.children[0].children[0].children[0].children[0].innerHTML = `Com. Positivos ${this.currentService.stats.indicatorsGlobal.comment[1]}% - Total ${this.currentService.stats.indicatorsGlobal.comment[0]}`

				const indicators = await getCanvas('indicatorsService')
				pdf.addImage(indicators.toDataURL('image/png'), 'PNG', 10, 95, 190, 30)
				pdf.text('Gráficos de Valoraciones por Tiempo', 49, 130)

				const chartHourService = document.getElementById('chartHourService')
				chartHourService.style.display = "block"
				
				const hour = await getCanvas('chartHourService')
				pdf.addImage(hour.toDataURL('image/png'), 'PNG', 10, 145, 190, 120)
				pdf.addPage('a4', 'portrait')

				const weekChartService = document.getElementById('weekChartService')
				weekChartService.style.display = "block"
				
				const weekChartPDF = await getCanvas('weekChartService')
				pdf.addImage(weekChartPDF.toDataURL('image/png'), 'PNG', 10, 20, 190, 120)

				const dayChartService = document.getElementById('dayChartService')
				dayChartService.style.display = "block"

				const dayChartPDF = await getCanvas('dayChartService')
				pdf.addImage(dayChartPDF.toDataURL('image/png'), 'PNG', 10, 150, 190, 120)
				pdf.addPage('a4', 'portrait')

				const monthChartService = document.getElementById('monthChartService')
				monthChartService.style.display = "block"

				const monthChartPDF = await getCanvas('monthChartService')
				...
				pdf.save(FILENAME)
				servicesFacesData2.style.display = "none"
				servicesIndSatisfaction.style.display = "none"
				chartHourService.style.display = "none"
				weekChartService.style.display = "none"
				dayChartService.style.display = "none"
				monthChartService.style.display = "none"
				this.loading2 = false
				this.loader2 = null
			},
			downloadXLSX(data) {
			...
			async dynamicDialog(data) {
				this.statsFacesService = (await this.$axios.post('services/stats/faces/value-prc', { 
					service: data.ftitle, condition: data.stats.filter || ` AND MONTH(assessments.date) = ${new Date().getMonth() + 1 } `})).data
			...
			async getChartGlobalDatesHour(data) {
				const hoursStats = (await this.$axios.post('/services/stats/hour', { service:  data.ftitle,
			...
			async getChartGlobalDatesDayW(data) {
				const dayWStats = (await this.$axios.post('/services/stats/dayW', { service:  data.ftitle,
				condition: data.stats.filter || ` AND MONTH(assessments.date) = ${new Date().getMonth() + 1 } `})).data
			...
			async getChartGlobalDatesMonth(data) {
				const monthStats = (await this.$axios.post('/services/stats/month', { service:  data.ftitle })).data
			...
			async getIndicatorsGlobal(data) {
				...
				const complains = (await this.$axios.post('/services/stats/complain', { service: data.ftitle,
				condition:  data.stats.filter || ` AND MONTH(date) = ${new Date().getMonth() + 1 } `})).data[0]

				const comments = (await this.$axios.post('/services/stats/comment', { service: data.ftitle,
				condition: data.stats.filter || ` AND MONTH(date) = ${new Date().getMonth() + 1 } `})).data[0]

				this.currentService.stats.indicatorsGlobal.complain = [complains.value, complains.percentage]
				this.currentService.stats.indicatorsGlobal.comment = [comments.value, comments.percentage]
				
				this.dynamicDialogAct = false
				this.dynamicDialogAct = true
			},
			async getChartGlobalDatesDay(data) {
				const dayStats = (await this.$axios.post('/services/stats/day', { service:  data.ftitle,
				condition: data.stats.filter || ` AND MONTH(assessments.date) = ${new Date().getMonth() + 1 } `})).data
