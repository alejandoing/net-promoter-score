<template lang="pug">
  .container
    .justification(v-for="reason in reasons")
          ...
          v-flex(xs3 offset-xs10)
            v-tooltip(top)
              v-btn(
                ...
                color="primary"
                slot="activator"
                @click="printGeneralReport(currentReason.stats.assessments)"
                :loading="loading2"
                :disabled="loading2"
              )
...
<script>
  ...
  methods: {
    async printGeneralReport(data) {
      ...
      const FILENAME  = 'reporte_motivos.pdf'
      let pdf = new jsPDF()
      ...
      pdf.setFontSize(20)
      pdf.text(`Reporte de Valoraciones (${this.currentReason.title})`, 35, 150)
      pdf.setFontSize(12)
